...
H.check("debuffs y: 2 above the one-pixel border", y, 3)
...
H.check("castbar", select(2, anchor(debuffs)), t.castbar.box)
C.Set("target", "castbarEnabled", false)
H.check("castbar off: frame", select(2, anchor(debuffs)), t.unitBox)
C.Set("target", "castbarEnabled", true)
C.Set("pet", "debuffsAnchor", "CASTBAR")
H.check("no castbar (pet): frame", select(2, anchor(ns.Frames.pet.auras.debuffs)), ns.Frames.pet.unitBox)
-- Groups anchored to each other: the debuffs hang from the frame.
C.Set("target", "debuffsAnchor", "OTHER")
H.check("cycle broken", select(2, anchor(debuffs)), t.unitBox)
...
H.check("chosen offset, below the border", x .. "," .. y, "-3,-6")
C.ResetScope("target")
