...
local function joined()
    return blockRing:IsShown() and not box:IsShown() and frameMask:IsShown()
        and frameMask._texture == ns.Corners.MaskFile(6, "TOP")
end
local function apart()
    return box:IsShown() and not blockRing:IsShown() and frameMask:IsShown()
        and frameMask._texture == ns.Corners.MaskFile(6, "ALL")
end
...
H.check("above: only the frame's bottom corners round", frameMask._texture, ns.Corners.MaskFile(6, "BOTTOM"))
ns.Castbar.Preview(f, false)
H.check("above, idle: all four round", frameMask._texture, ns.Corners.MaskFile(6, "ALL"))
...
H.check("low frame: mask clamped", f.clip.mask._slice[1], 4)
...
H.check("low castbar: frame mask", f.clip.mask._slice[1], 5)
H.check("low castbar: castbar mask", bar.clip.mask._slice[1], 5)
...
H.check("detached: radius clamped to the castbar (16 high)", bar.clip.mask._slice[1], 8)
